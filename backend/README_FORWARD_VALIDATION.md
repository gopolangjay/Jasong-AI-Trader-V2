...
Provenance
The V6.11 FX and XAUUSD strategies use the resilient routed-data path for closed-candle structure; the execution/preflight quote remains a fresh IG DEMO quote. Only the structural price distance is transferred to that quote. The registry records analysis and broker sources separately and exposes missing or stale broker quotes instead of silently treating `UNAVAILABLE` as usable.
