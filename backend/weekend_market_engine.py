...
VERSION = "6.13-weekend-structure-execution-v4"
...
def _price(row: Dict[str,Any], field: str) -> Optional[float]:
    ...
    if isinstance(b,dict):
        vals=[_num(b.get(k)) for k in ("bid","ask","offer","lastTraded")]; vals=[x for x in vals if x is not None]
...
def structure_signal(m5: List[Dict[str,float]],m1: List[Dict[str,float]],spread: float)->Dict[str,Any]:
    ...
    recent=m1[-7:-1]; tol=max(spread*1.5,abs(level)*.00015)
    retest=any(x["low"]<=level+tol and x["close"]>=level-tol for x in recent) if direction=="BUY" else any(x["high"]>=level-tol and x["close"]<=level+tol for x in recent)
...
def _rule_distance(rule: Any, reference: float) -> Optional[float]:
    ...
    return reference*value/100.0 if "PERCENT" in unit else value

class WeekendMarketEngine:
    def __init__(self,broker:Any)->None:
        self.broker=broker; self.enabled=str(os.getenv("WEEKEND_AUTOTRADE","true")).lower() in {"1","true","yes","on"}; self.poll_seconds=max(60,int(os.getenv("WEEKEND_SCAN_SECONDS","180"))); self.max_open=max(1,min(2,int(os.getenv("WEEKEND_MAX_OPEN_POSITIONS","2")))); self.max_spread_bps=max(5.0,float(os.getenv("WEEKEND_MAX_SPREAD_BPS","100"))); self.default_size=max(.0001,float(os.getenv("WEEKEND_DEFAULT_SIZE","0.5"))); self.friday_start_utc=max(0,min(23,int(os.getenv("WEEKEND_FRIDAY_START_UTC","18")))); self._stop=threading.Event(); self._thread=None
        self._state={"version":VERSION,"strategy_id":STRATEGY_ID,"enabled":self.enabled,"last_scan_at":None,"last_error":None,"markets":[],"signals":[],"market_discovery":[],"execution_attempts":[],"opens":0}
    ...
    def _owned_open(self)->List[Dict[str,Any]]:
        return [i for i in (self.broker.positions() or {}).get("positions",[]) or [] if str((i.get("position") or {}).get("dealReference") or "").upper().startswith("JSWKND_")]
    def _resolve(self,seed:Dict[str,Any])->Tuple[Optional[Dict[str,Any]],Dict[str,Any]]:
        d={"symbol":seed["symbol"],"terms":seed["terms"],"eligible":False,"stage":"SEARCH"}
        try:
            m=self.broker.resolve_global_market(search_terms=list(seed["terms"]),name_tokens=list(seed["tokens"]),require_tradeable=True,cache_key="WKND_"+seed["symbol"]); d.update({"stage":"DETAILS","epic":m.get("epic"),"resolved_name":m.get("name"),"search_market_status":m.get("market_status"),"search_instrument_type":m.get("instrument_type")}); details=self.broker.market_details(str(m["epic"]),require_quote=True); ins,snap=details.get("instrument") or {},details.get("snapshot") or {}; q=self.broker.extract_snapshot_quote(details); s={"epic":m["epic"],"instrumentName":ins.get("name") or m.get("name"),"instrumentType":ins.get("type") or m.get("instrument_type"),"category":"CRYPTO","marketStatus":snap.get("marketStatus") or m.get("market_status"),"bid":q.get("bid"),"offer":q.get("offer"),"symbol":seed["symbol"],"expiry":ins.get("expiry") or m.get("expiry") or "-"}; v=assess_market(s); d.update({"stage":"POLICY","market_status":s.get("marketStatus"),"instrument_type":s.get("instrumentType"),"bid":s.get("bid"),"offer":s.get("offer"),"eligible":bool(v.get("eligible")),"reason":v.get("reason")}); return (s if v.get("eligible") else None),d
        except Exception as e:d.update({"eligible":False,"reason":"DISCOVERY_EXCEPTION","error_type":type(e).__name__,"error":str(e)[:500]});return None,d
    def _signal(self,m:Dict[str,Any])->Dict[str,Any]:
    ...
    def _execute(self,sig:Dict[str,Any])->Dict[str,Any]:
        ...
        bid,offer=float(q["bid"]),float(q["offer"]); entry=offer if sig["direction"]=="BUY" else bid; rules=details.get("dealingRules") or {}; min_stop=_rule_distance(rules.get("minNormalStopOrLimitDistance"),entry) or 0.0; min_control=_rule_distance(rules.get("minControlledRiskStopDistance"),entry) or 0.0; broker_min=max(min_stop,min_control,offer-bid); structural_risk=abs(entry-float(sig["stop"])); risk=max(structural_risk,broker_min*1.05)
        stop=entry-risk if sig["direction"]=="BUY" else entry+risk; target=entry+risk*1.5 if sig["direction"]=="BUY" else entry-risk*1.5
        if risk<=0:raise RuntimeError("INVALID_BROKER_ADJUSTED_RISK")
        size=self.broker._normalise_deal_size(self.default_size,minimum_size=self.broker._min_deal_size(details),increment=self.broker._deal_size_increment(details)); payload={"currencyCode":self.broker._default_currency(ins),"dealReference":("JSWKND_"+uuid.uuid4().hex[:20])[:30],"direction":sig["direction"],"epic":sig["epic"],"expiry":str(ins.get("expiry") or "-"),"forceOpen":True,"guaranteedStop":False,"orderType":"MARKET","size":round(size,12),"stopLevel":round(stop,10),"limitLevel":round(target,10)}; ack=self.broker._request("POST","/positions/otc",version=2,payload=payload); ref=str(ack.get("dealReference") or payload["dealReference"]); conf=self.broker.confirm(ref)
        result={"dealReference":ref,"dealId":conf.get("dealId"),"dealStatus":conf.get("dealStatus"),"direction":sig["direction"],"entry":entry,"stop":stop,"target":target,"target_r":1.5,"structural_risk":structural_risk,"broker_min_distance":broker_min,"risk_adjusted_for_broker":risk>structural_risk}
    ...
    def tick(self)->Dict[str,Any]:
        ...
        try:
            ...
            resolved=[self._resolve(s) for s in CRYPTO_SEEDS]; markets=[m for m,_ in resolved if m]; discovery=[d for _,d in resolved]; signals=[self._signal(m) for m in markets]; qualified=sorted([s for s in signals if s.get("eligible")],key=lambda x:(float(x.get("spread_bps") or 999999),str(x.get("symbol") or "")))
            self._state.update({"markets":markets,"market_discovery":discovery,"signals":signals,"execution_attempts":[],"last_error":None})
            attempts=[]; successes=0; slots=self.max_open-len(opens)
